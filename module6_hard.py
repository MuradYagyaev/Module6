...
class Figure:
    ...
    def set_sides(self, *sides):
        if isinstance(self, (Circle, Triangle)) and self.__is_valid_sides(sides):
            self.__sides = list(sides)
        if isinstance(self, Cube) and self.__is_valid_sides(sides):
            for i in range(self.sides_count):
...
class Circle(Figure):
    ...
    def __init__(self, *param):
        ...
        self.__radius = self.__set_radius()
...
print('Треуголник, цвет:', triangle2.get_color())
...
print('Площадь тругольника:', triangle2.get_square())
print('Высота тругольника:', triangle2.get_height())
